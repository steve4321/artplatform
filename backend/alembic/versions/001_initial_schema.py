...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "asset_type IN ('model_3d', 'texture_2d', 'sprite', 'material', "
            "'animation_clip', 'prefab', 'audio', 'vfx')",
            name="ck_assets_asset_type",
        ),
        sa.CheckConstraint(
            "source IN ('ai_generated', 'manual_upload', 'hybrid')",
            name="ck_assets_source",
        ),
        sa.CheckConstraint(
            "state IN ('draft', 'processing', 'review', 'approved', "
    ...
    op.create_index("idx_assets_team_state", "assets", ["team_id", "state"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=True),
        sa.CheckConstraint(
            "source_type IN ('ai_pipeline', 'manual_upload', 'edited')",
    ...
    op.create_table(
        ...
        sa.Column("completed_stages", sa.Integer(), server_default="0", nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=True),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "dependency_type IN ('references_texture', 'references_material', "
            "'references_rig', 'references_animation')",
    ...
    op.create_table(
        ...
        sa.Column("input_artifact_ids", postgresql.ARRAY(sa.UUID()), server_default="'{}'", nullable=True),
        sa.Column("output_artifact_ids", postgresql.ARRAY(sa.UUID()), server_default="'{}'", nullable=True),
